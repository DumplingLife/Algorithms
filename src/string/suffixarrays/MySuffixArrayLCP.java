...
public class MySuffixArrayLCP
{
	public static void main(String[] args) throws FileNotFoundException {
		MyTimer timer = new MyTimer();
		Scanner scan = new Scanner(new File("test.in"));
		SuffixArrayLCP sa = new SuffixArrayLCP(scan.nextLine());
		System.out.println(Arrays.toString(sa.suffixArr));
		
		System.out.println(sa.countDistinctSubstrings());
		timer.lap();
	}
	
	
	
	static class SuffixArrayLCP {
		public String s;
		public int N;
		...
		public SuffixArrayLCP(String s) {
			this.s = s;
			N = s.length();
		...
		private int next(int x) {
			return suffixArr[rank[x]+1];
		}
		
		public int countDistinctSubstrings() {
			int ret = 0;
			for(int i = 0;i<N;i++) {
				int suffixLength = N - suffixArr[i];
				ret += suffixLength - (i == 0 ? 0 : LCPs[i-1]);
			}
			return ret;
		}
	}
